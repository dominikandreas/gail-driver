...
# specifies architecture of "feature" networks
parser.add_argument('--hspec', type=int, nargs='+',
                    default=[256, 128, 64, 64, 32])
parser.add_argument('--p_hspec', type=int, nargs='+',
                    default=[])  # policy layers
parser.add_argument('--b_hspec', type=int, nargs='+',
                    default=[])  # baseline layers
parser.add_argument('--r_hspec', type=int, nargs='+',
                    default=[])  # reward layers
...
reward = RewardMLP('mlp_reward', 1, r_hspec, nonlinearity, tf.nn.sigmoid,
                   input_shape=(
                       np.prod(env.spec.observation_space.shape) + env.action_dim,)
                   )
